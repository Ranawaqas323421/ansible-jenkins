# Ansible + Jenkins Multi-Server Deployment Pipeline
...
1. **Checkout** — Jenkins pulls the latest playbook code from GitHub.
2. **Install Collections** — `ansible-galaxy collection install -r requirements.yml` installs required Ansible collections (`community.docker`, `community.mysql`).
3. **Run Playbook** — `ansible-playbook -i inventory/hosts site.yml` executes two plays:
